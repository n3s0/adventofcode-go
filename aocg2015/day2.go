...
*/
func dayTwoPartOne(input string) (totalSquareFeet int) {
    ...
    for _, dimension := range dimensions {
        ...
        slicedDimensions := strings.Split(dimension, "x")

        l, _ = strconv.Atoi(slicedDimensions[0])
        w, _ = strconv.Atoi(slicedDimensions[1])
        h, _ = strconv.Atoi(slicedDimensions[2])

        lquot = l * w
        wquot = w * h
        hquot = h * l

        num := []int{lquot, wquot, hquot}

        if len(num) == 0 {
            fmt.Println("No input in numbers")
        }
        ...
        total := (2*lquot) + (2*wquot) + (2*hquot) + lowest
...
feet.
*/
func dayTwoPartTwo(input string) (totalRibbon int) {
    var wrappingRibbon int
    var bowRibbon int
    dimensions := strings.Split(input, "\n")
    ...
    for _, dimension := range dimensions {
        ...
        wrappingRibbon = dims[0] + dims[0] + dims[1] + dims[1]
        bowRibbon = dims[0] * dims[1] * dims[2]

        total := wrappingRibbon + bowRibbon
...
14x6x11`
    ...
    partone := dayTwoPartOne(input)
    parttwo := dayTwoPartTwo(input)
