...
func dayOnePartOne(puzzleInput string) (solution int) {
    fmt.Println("--- Day 1: Not Quite Lisp / Part 1 ---")

    var floor int

    puzzle := strings.Split(puzzleInput, "")

    floor = 0
    for _, puz := range puzzle {
        if puz == "(" {
            floor += 1
        }
...
func dayOnePartTwo(puzzleInput string) (solution int) {
    fmt.Println("--- Day 1: Not Quite Lisp / Part 2 ---")
...
// the input is on new lines. Need to split it into an array.
// Then need to split the length, width, and height
// make the calculation of each 2*l*w + 2*w*h + 2*h*l + area of smallest
func dayTwo(puzzleInput string) {
    //var total int
